...
| `?inner_content` | `az:inner_content(Bindings)` -- the rendered page a layout wraps. **Opaque, content slot only**: an internal nested template (so the page is spliced, not copied and re-decoded per layer), not iodata -- `iolist_size` on it raises `badarg`, and in an attribute value it raises `bad_template_value` |
