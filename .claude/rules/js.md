...
- `resolveEl(target)` -- splits `"viewId:az"`, finds `getElementById(viewId)` then `[az="az"]`. Two fallbacks when no element carries the az: a compound `"X:n"` slot resolves to the base `[az="X"]` element, and otherwise the slot's `<!--az:X-->` comment marker is located in the view subtree and its **parent element** returned. The marker fallback is what makes a **marker-only** slot patchable -- a template whose whole body is a bare dynamic (`?html(case ... end)`, `?html(?get(x))`, `?html(?each(...))`, `?html(?stateless(...))`, a mixed top-level fragment) anchors its root slot with its own marker pair that no element carries, so without it every op addressed to such a component is dropped.
