...
**Lifecycle:**
- `mounted()` -- on `ws.onopen`, after `OP_INSERT`/`OP_UPDATE`/`OP_REPLACE`/`OP_TEXT` (marker path **and** markerless-HTML path)/`OP_LIST_PATCH` inserts. Guarded -- never double-fires.
...
- `destroyed()` -- before `OP_REMOVE_NODE`/`OP_REMOVE`/`OP_REPLACE`/`OP_UPDATE`/`OP_TEXT`/`OP_LIST_PATCH` removals. Called BEFORE DOM mutation, while the element is still attached.
