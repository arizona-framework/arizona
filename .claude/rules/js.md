...
- **Effects body.** Whenever the response body parses as `{ e: effects }`, `applyEffects(effects)` runs -- **regardless of status**, so a controller can return a real `4xx` (e.g. `422`) and still drive inline validation. An empty `2xx` body (a `204` cookie-only response that re-syncs via pubsub) applies nothing. `Set-Cookie` is applied natively by the browser. (No response-effect view-transition wrapping -- a follow-up; pubsub-driven re-renders animate through the normal WS path.)
