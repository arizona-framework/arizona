...
- **Effects body.** Whenever the response body parses as `{ e: effects }`, `applyEffects(effects)` runs -- **regardless of status**, so a controller can return a real `4xx` (e.g. `422`). An empty `2xx` body (a `204` cookie-only response that re-syncs via pubsub) applies nothing. `Set-Cookie` is applied natively by the browser. (No response-effect view-transition wrapping -- a follow-up; pubsub-driven re-renders animate through the normal WS path.)
...
- **`push_event` in the response (default, the submitting view).** Return `arizona_js:push_event(~"...")`; the fetch handler runs response effects against the **trigger element**, so `resolveTarget` finds the form's enclosing view (root **or** a child view -- use `az-target` to aim elsewhere) and the client relays the event over its WS -> `handle_event/3` re-renders. It auto-collects the form payload (the controller's result isn't in the form, so pass it as an explicit payload -- it merges on top). Success leg: `reply_effects/1` (200, the form auto-resets). Error leg: `reply_effects(Status, Effects)` with a non-2xx (e.g. `422`) so the typed fields survive while the effects still apply. No subscription, works for anonymous forms. Fixture: `arizona_fetch_push`.
...
**Selector targeting:** the broadcast effects (`toggle`/`show`/`hide`/`add_class`/`remove_class`/`toggle_class`/`set_attr`/`remove_attr`/`toggle_attr`) act on **all** elements matching the selector, via `withQueryAll` (`querySelectorAll` across the main + PiP documents). The single-target effects (`focus`/`blur`/`scroll_to`) act on the **first** match only, via `withQuery`.
