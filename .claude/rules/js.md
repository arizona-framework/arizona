...
- **`push_event` in the response (default, the submitting view).** Return `arizona_js:push_event(~"...")`; since the fetch handler runs response effects against the **trigger element**, `resolveTarget` finds the submitting view and the client relays the event over its WS -> `handle_event/3` re-renders. No subscription, targets the exact view, works for anonymous forms. Fixture: `arizona_fetch_push`.
