...
**Showing content is server-authoritative, not a response effect** -- there is deliberately no `set_text`/`set_html` (it would fight the diff engine), and the response is effects-only because a stateless controller has no diff snapshot to make ops from (hence `accept: application/json`, no ops). To update **the submitting view**, return an `arizona_js:push_event` in the response: the client relays it over the existing WebSocket and the view re-renders through its normal `handle_event/3` -- no subscription, works for anonymous forms. It targets the form's enclosing view (root **or** a child view; `az-target` to redirect) and auto-collects the form payload (the controller's result isn't in the form, so pass it as an explicit payload -- it merges on top). Success uses `reply_effects/1` (200, the form auto-resets); an error leg uses `reply_effects(Status, Effects)` with a non-2xx (e.g. `422`) so the typed fields survive while the effects still apply. Use `arizona_pubsub` instead when the change must reach **other** views/users (broadcast), scoping the topic by user/session (e.g. `{account, UserId}`). Either way the view renders from state via the diff; the response effects themselves are for *request-local imperative UI* only (toggling a pre-rendered element, focus). The two mechanisms have working e2e fixtures: `arizona_fetch_push` (push_event) and `arizona_fetch_account` (pubsub).
