...
Cost note: `insert/2` and `update/3` are O(1) amortized under `halt`/`infinity`. Under **`on_limit => drop` at capacity** every append instead evicts, which flattens the order and discards the append buffer, so each one is O(Limit) -- measured ~0.7us vs ~38us per insert at `limit => 16000`. That is the mode a bounded tail-buffer uses, so size the limit accordingly; `delete/2`, `insert/3`, `move/3` are O(N) list surgery on the order, so K per-item calls in one event are O(K*N). For bulk mutation build the list once and `reset/2` (one op -- the differ still patches only the real per-item delta) or reorder with one `sort/2`.
