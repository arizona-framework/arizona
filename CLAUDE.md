...
**Any element event works.** `az-<event>` binds `addEventListener(<event>)` with the attribute suffix used verbatim as the listener type, so there is no supported-event list: `az_toggle` on a `<dialog popover>`, `az_close` on a `<dialog>`, `az_error` on an `<img>`, and a custom element's own `az_sl_change` all bind without registering anything. Non-bubbling events (`toggle`, `close`, `scroll`, `play`, `load`, ...) are delivered too, matched on the element that declares them rather than by ancestor lookup, since such an event has no propagation path. Types are discovered from the markup as it arrives, so an event that only a later patch introduces still binds. The name must be lowercase (the HTML parser lowercases attribute names), which is what custom elements use anyway. Two families are out of reach by construction: **window**-targeted events (`resize`, `online`, `hashchange`) never reach the Document delegation binds on, and a subresource `load`/`error` has usually fired before discovery runs on the WebSocket open. Mechanism, those limits, and why `submit`/`drop` keep dedicated listeners, in [.claude/rules/js.md](.claude/rules/js.md).
...
**Neither mode can change the layout, so a route that changes it degrades to a full page load.** `layouts` render **once, at SSR**; `OP_REPLACE` swaps only the view *inside* them and a patch keeps the shell outright, so no frame can re-render a layout. `arizona_socket` therefore compares the resolved target route's `layouts` against the ones already on screen (tracked from the connect route, threaded `arizona_ws:prepare/3` -> `arizona_socket:init/4`) and, when they differ, ships `arizona_js:navigate(Url, #{full => true})` instead -- the browser loads the URL for real, layouts and all. Same degradation ladder that already turns a cross-handler `patch` into a navigate and an unresolvable target into a full load, and it covers **both** frame kinds plus the middleware halt-redirect path (whose navigate effect comes back as an ordinary navigate frame). The comparison is term equality on the whole list, deliberately: `apply_layouts/3` nests them (`[Root, Section]` renders `Root(Section(Page))`), so an inner layer wraps the replaced view exactly as the outer one does and a difference at any depth disqualifies an in-place swap. Practical upshot: **crossing layout families needs no hand-annotated `<a href>`** -- link with `az_navigate`/`az_patch` and the framework picks the right kind, including for a redirect the *server* issues, which app-side annotation could never cover.

A pending flash cannot ride a full page load (a WS frame has no `Set-Cookie` leg), so its fate depends on whether anything reproduces it. A flash a **halting middleware** set is reproducible: the socket remembers the gated path beside it and points the full navigation *there*, so the middleware runs again over HTTP and redirects with a real signed flash cookie -- the browser lands on the target with both its own layouts and the message (cost: that path's middlewares run twice, which any GET route already tolerates from a refresh or prefetch). That reroute is armed for **only the redirect's own target**; a full navigation somewhere else is the user going somewhere else, and gets taken at face value rather than hijacked back through the gate. A flash an **in-view handler** set (an `arizona_js:navigate`/`patch` `flash` opt) has no request-side generator, so it is dropped with a warning naming the remedy (set it from a middleware on the requested path, or keep it in the session).

**Layouts still never re-render when they match.** Same-layout navigation keeps the shell exactly as SSR built it, so anything the shell derives from bindings (a per-route `<title>`) stays frozen and remains the view's job to patch -- `arizona_js:set_title/1`. The degradation is about layout *identity*, not layout *freshness*.
...
**Server-side store (opt-in).** Set the `session_store` app env to an `arizona_session_store` module (the default `arizona_session_store_ets`, or your own Redis/Mnesia backend for multi-node) and the `az_session` cookie carries only a **signed opaque id** (with the same `session_max_age` expiry baked into the signature, so a captured cookie cannot outlive it even if the store's own expiry never fires) while the map lives in the store -- enabling **revocation** (`Store:delete(Id)`), large/secret state, and data the client never sees. The mode switch lives in `arizona_req` (`read_session` resolves the id through the store; `commit_session/1`, called by the transports at response flush, persists -- `resp_cookies/1` is a pure getter that only builds the cookie list); the map-level API (`put_session`/`get_session`/`session`) is unchanged. `arizona_req:session_id/1` exposes the id so an app can record it to revoke later. **Login rotation (fixation defense):** at a privilege boundary do `clear_session/1` then `put_session/3` (the logout-then-login pattern) -- store mode mints a **fresh** id for the write rather than reusing the incoming one and drops the pre-login entry, so a pre-login (possibly attacker-planted) id never becomes the authenticated session; a plain `put_session/3` without a preceding `clear_session/1` updates in place under the same id (correct for an already-authenticated user). Cookie mode is not affected (no server-side id to fixate). The ETS store is a supervised table owner (started via `arizona_sup` when configured) with a periodic sweep (`session_store_sweep_ms`, default 60000) plus lazy expiry; mirrors `Plug.Session.Store`. The `commit_session` persist is **commit-on-success** -- the store write happens at response flush (the transports call it beside serializing `resp_cookies`), so a handler that crashes after `put_session/3` leaves the store untouched. **Preconditions:** store mode needs `secret_key` (the id is *signed*, not just random) and the `session_store` env set **before the app boots** (it's read at `arizona_sup` init; setting it later leaves the store unsupervised and session ops crash on the missing table). A controller writes store-mode sessions through the same `req/1` -> write -> `put_req/2` round trip as cookie mode (the flush runs `commit_session/1`, so the store put/delete and the cookie always agree); dropping to `arizona_session_store_ets:put/3` plus `arizona_session:set_cookie_id/1` + `roadrunner_resp:set_cookie/4` is the escape hatch, not the path. Fixed per-write TTL (`session_max_age`); **WS-activity (sliding) renewal and per-user revocation are deferred** -- the store makes them possible (renew the entry on a live event, no cookie needed) but they need live-process wiring, a separate change.
