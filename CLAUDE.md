...
**Showing content is server-authoritative, not a response effect.** Effects only toggle attrs/classes/visibility -- there is deliberately no `set_text`/`set_html` (that would fight the diff engine), and the response is effects-only because a stateless controller has no diff snapshot to make ops from (hence `accept: application/json`, no ops). To show a validation message or success text, the controller **broadcasts it over `arizona_pubsub`** to a topic the view subscribed to in `mount/1` -- scope the topic by user/session (e.g. `{account, UserId}`) so it reaches the right view -- and the view renders it from state via normal diffing. The fetch response effects are for *request-local imperative UI* only (e.g. toggling a pre-rendered element, focus).
