...
| `src/arizona_middleware.erl`              | Request-to-bindings middleware pipeline -- `apply_middlewares/3` runner + built-in `extract/1`/`put_request/2`/`fetch_flash/2`/`fetch_session/2`/`check_origin/2` steps, plus the HTTP response transform constructors (`etag/0`, `compress/0`, `cors/1`, `security_headers/0,1`) the router compiles into the server's response pipeline |
...
| `src/arizona_dev_log.erl`                 | Bounded log ring behind the dev MCP's `get_logs` tool -- a `logger` handler keeping the last 1024 formatted events; table owned by `arizona_sup`, handler installed on first MCP session  |
