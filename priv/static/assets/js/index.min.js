...
import { default as h, LOG_LEVELS as l } from "./logger/arizona-logger.min.js";
...
class o {
  ...
  applyDiffValue(t, r, i) {
    ...
    if (s?.type === "stateful") {
      ...
      if (Array.isArray(i)) {
        const e = this.structure.get(s.id);
        if (!e) {
          const n = String(s.id).replace(/\r|\n/g, "");
    ...
    if (Array.isArray(i)) {
      ...
      if (s?.type === "stateless") {
        i.forEach(([e, n]) => {
          this.applyDiffValue(s.dynamic, e - 1, n);
        });
        return;
      }
      t[r] = i;
