...
import { default as y, LOG_LEVELS as l } from "./logger/arizona-logger.min.js";
import { default as S } from "./logger/arizona-console-logger.min.js";
...
class o {
  ...
  applyDiff(t, r) {
    ...
    const s = this.structure.get(t);
    for (const [i, e] of r)
      this.applyDiffValue(s.dynamic, i - 1, e);
  }
  /**
   ...
   * @returns {void}
   */
  applyDiffValue(t, r, s) {
    const i = t[r];
    if (i?.type && Array.isArray(s))
      switch (i.type) {
        case "stateful": {
          const e = this.structure.get(i.id);
          if (!e) {
            const n = String(i.id).replace(/\r|\n/g, "");
            console.warn(`[Arizona] Component '${n}' not found in structure`);
            return;
          }
          s.forEach(([n, a]) => {
            this.applyDiffValue(e.dynamic, n - 1, a);
          });
          return;
        }
        case "stateless":
          s.forEach(([e, n]) => {
            this.applyDiffValue(i.dynamic, e - 1, n);
          });
          return;
        case "list":
          i.dynamic = s;
  ...
  generateStatefulHTML(t) {
    ...
    if (!r) {
      const i = String(t).replace(/\r|\n/g, "");
      throw console.warn(`[Arizona] StatefulId '${i}' not found in structure`), new Error(`Component ${i} not found`);
    }
   ...
   */
  generateListHTML(t) {
    const { static: r, dynamic: s } = t;
    return s.reduce((i, e) => i + this.zipStaticDynamic(r, e), "");
  }
  /**
  ...
  zipStaticDynamic(t, r) {
    const s = [], i = Math.max(t.length, r.length);
    for (let e = 0; e < i; e++)
      e < t.length && s.push(t[e]), e < r.length && s.push(this.normalizeDynamicElement(r[e]));
    return s.join("");
  }
  /**
...
export {
  p as Arizona,
  S as ArizonaConsoleLogger,
  o as ArizonaHierarchical,
  y as ArizonaLogger,
  l as LOG_LEVELS
};
