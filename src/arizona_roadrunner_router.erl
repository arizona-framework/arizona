...
compile_routes(Routes) ->
    RoadrunnerRoutes = lists:flatmap(fun route_to_roadrunner/1, Routes),
...
-spec routes(Routes) -> [roadrunner_router:route()] when
    Routes :: [route()].
routes(Routes) ->
    lists:flatmap(fun route_to_roadrunner/1, Routes).
...
%% the wrong signature.
route_to_roadrunner({live, Path, Handler, Opts}) ->
    [
...
route_to_roadrunner({ws, Path, Opts}) ->
    [{Path, arizona_roadrunner_ws, #{arizona => Opts}}];
route_to_roadrunner({asset, Path, {dir, Dir}}) ->
    asset_route(Path, Dir);
route_to_roadrunner({asset, Path, {priv_dir, App, SubDir}}) ->
    asset_route(Path, filename:join(code:priv_dir(App), SubDir));
route_to_roadrunner({controller, Path, Handler, State}) ->
    [{Path, Handler, State}];
route_to_roadrunner({reload, Path, Opts}) ->
    persistent_term:put(arizona_reload_url, Path),
    [{Path, arizona_roadrunner_reload, #{arizona => Opts}}].

asset_route(Path, Dir) ->
    [{<<Path/binary, "/*path">>, arizona_roadrunner_static, #{arizona => #{dir => Dir}}}].
