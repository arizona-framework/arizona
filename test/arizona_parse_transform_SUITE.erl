...
test_extract_module_name(Config) when is_list(Config) ->
    ...
    ?assertEqual(test_module, arizona_parse_transform:extract_module_name(Forms1)),

    % Test with module attribute not first
    Forms2 = [{attribute, 1, export, []}, {attribute, 2, module, other_module}],
    ?assertEqual(other_module, arizona_parse_transform:extract_module_name(Forms2)),

    % Test with no module attribute
    Forms3 = [{attribute, 1, export, []}],
    ?assertEqual(undefined, arizona_parse_transform:extract_module_name(Forms3)),

    % Test with empty forms
    ?assertEqual(undefined, arizona_parse_transform:extract_module_name([])),
    ok.
...
test_analyze_application(Config) when is_list(Config) ->
    ...
    Result = arizona_parse_transform:analyze_application(AppNode),
    ...
    ?assertEqual(undefined, arizona_parse_transform:analyze_application(LocalCall)),

    % Test with non-atom module
    InvalidModule = {call, 1, {remote, 1, {var, 1, 'Module'}, {atom, 1, func}}, []},
    ?assertEqual(undefined, arizona_parse_transform:analyze_application(InvalidModule)),
    ok.

test_eval_expr(Config) when is_list(Config) ->
    % Test string evaluation
    StringExpr = {string, 1, "hello"},
    Result = arizona_parse_transform:eval_expr(test_module, StringExpr, #{}),
    ?assertEqual("hello", Result),

    % Test binary evaluation
    BinaryExpr = {bin, 1, [{bin_element, 1, {string, 1, "test"}, default, default}]},
    BinaryResult = arizona_parse_transform:eval_expr(test_module, BinaryExpr, #{}),
    ?assertEqual(~"test", BinaryResult),
    ok.

test_error_info(Config) when is_list(Config) ->
    Cause = {test_module, 42, error, some_reason, []},
    ErrorInfo = arizona_parse_transform:error_info(Cause),

    ?assertMatch(
        [{error_info, #{cause := Cause, module := arizona_parse_transform}}],
        ErrorInfo
    ),
    ok.
