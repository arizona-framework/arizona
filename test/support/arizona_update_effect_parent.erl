...
mount(Bindings) ->
    {#{id => maps:get(id, Bindings, ~"uep"), value => 0, label => ~"L"}, #{}}.
...
%% Changes only `label` -- the child slot's deps (`value`) stay untouched, so a
%% dep-gated child diff must skip the child descriptor (no handle_update run).
